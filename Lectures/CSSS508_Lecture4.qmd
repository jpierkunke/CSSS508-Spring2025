...
## Pros to using R over spreadsheets
...
## A running example for today {.scrollable}
...
## Logicals with our example {.scrollable}
...
## Using logical indices to subset your data
...
## Tip: Making simple test examples to understand how a function works
...
## Conditional transformations {.scrollable}
...
## Using `ifelse` and `case_when` to add/modify columns
...
## The `nycflights13` datasets {.scrollable}
...
## `distinct()` drops variables! {.scrollable}
...
## Grouping by Multiple Variables {.scrollable} 
...
## Create New Columns: `mutate()` {.scrollable}
...
## Select Specific Rows Per Group: `slice_*` {.scrollable}
...
## Rename Variables: `rename()` {.scrollable}
...
2. `rename(new_name = old_name)` is the format. You can try out `janitor::clean_names()` if you want to automate this process for a lot of variables. 
...
# Practice: Manipulating Data{.section-title background-color="#99a486"}
...
1. Sort the rows by year (ascending order) and population (descending order) and overwrite the object you created for answer 1. Print the first 6 rows.
...
1. **Primary**: a variable or set of variables that *uniquely identifies* each observation.
    i) Called a **compound key** if it requires multiple variables to uniquely identify each observation
    ii) You can use `unique()`, `n_distinct()`, `distinct()`, `select()` and related functions to see which variables or combinations of variables uniquely identify each row; how?
...
## {data-menu-title="Homework 4" background-iframe="https://jpierkunke.github.io/CSSS508-Spring2025/Homework/homework4.html" background-interactive=TRUE}
...
# Extras{.section-title background-color="#1e4655"}


## Unexpected Behavior: `==` {.scrollable}
...
## Unexpected Behavior: Missing values {.scrollable}
...
## Examples of Logical Operators {.scrollable}
