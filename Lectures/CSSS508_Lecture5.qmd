...
## A Common Issue with Reading in Data
...
1. You can short-code column types with `c` = character, `t` = time, `D` = date, `i` = integer. <br> The `collapse` argument collapses the first two arguments into one complete character string.
...
`tribble()` (`tr`ansposed ti`bble`) allows you to lay out the data by rows, the way a dataframe looks.
...
- Move this data to the same folder where your source document^[`R` Script or `.qmd`] will be saved for this lab.
...
- [Go to the CSSS 508 Canvas page > Files.]{.semi-transparent}
- [Download the data folder or open the folder and download the individual data files.]{.semi-transparent}
- [Move this data to the same folder where your source document^[`R` Script or `.qmd`] will be saved for this lab.]{.semi-transparent}
...
## Creating Dates/Date-Times {.scrollable} 
...
## Changing Time Zones {.scrollable}
...
## Answers {.scrollable} {visibility="hidden"} 
...
## {data-menu-title="Homework 5" background-iframe="https://jpierkunke.github.io/CSSS508-Spring2025/Homework/homework5.html" background-interactive=TRUE}
